...
export default function VisualHiveStack({
  ...
  const [hoveredComponent, setHoveredComponent] = useState<Doc<"hiveComponents">["identifier"] | null>(null);
  ...
  const getComponentStackingHeight = (type: Doc<"hiveComponents">["type"]) => {
    switch (type) {
      ...
      case "Königinnenabsperrgitter": return 15;
      case "One Way Gate": return 20;
      case "Futterraum": return 70;
      default: return 70;
    }
  };

  const getComponentVisualRenderHeight = (type: Doc<"hiveComponents">["type"]) => {
    switch (type) {
      case "Deckel": return 30;
      case "Zarge": return 64;
      case "Boden": return 38;
      case "Königinnenabsperrgitter": return 14;
      case "One Way Gate": return 19;
      case "Futterraum": return 64;
      default: return 64;
    }
  };
  ...
  const getComponentColour = (component: Doc<"hiveComponents">) => {
    switch (component.type) {
      ...
      case "Königinnenabsperrgitter": return themeColors.Koeniginnenabspehrgitter;
      case "One Way Gate": return themeColors.OneWayGate;
      case "Zarge": return themeColors.Zarge;
      case "Futterraum": return themeColors.Futterraum;
  ...
  const renderInnerComponent = (component: Doc<"hiveComponents">) => {
    switch (component.type) {
      case "Deckel": return <DeckelSVG height={25} fill={getComponentColour(component)} isHovered={hoveredComponent === component.identifier} />;
      case "Boden": return <BodenSVG height={30} fill={getComponentColour(component)} isHovered={hoveredComponent === component.identifier} />;
      case "Königinnenabsperrgitter": return <QueenExcluderSVG height={14} fill={getComponentColour(component)} isHovered={hoveredComponent === component.identifier} />;
      case "One Way Gate": return <OneWayGateSVG height={19} fill={getComponentColour(component)} isHovered={hoveredComponent === component.identifier} />;
      case "Zarge":
      case "Futterraum":
        return <ZargeSVG
    ...
    return [...components].sort((a, b) => {
      ...
      if (posA?.type === posB?.type) {
        if (posA?.type === "forceTop" && posB?.type === "forceTop") {
          return ((posA as { type: "forceTop"; forceFromTop: number; }).forceFromTop ?? Infinity) - ((posB as { type: "forceTop"; forceFromTop: number; }).forceFromTop ?? Infinity);
        }
        if (posA?.type === "forceBottom" && posB?.type === "forceBottom") {
          return ((posB as { type: "forceBottom"; forceFromBottom: number; }).forceFromBottom ?? Infinity) - ((posA as { type: "forceBottom"; forceFromBottom: number; }).forceFromBottom ?? Infinity);
        }
        return 0;
      }
            ...
            return sortedComponents.map((component) => {
              ...
              return (
                <Link
                  ...
                  className={cn("group", { "pointer-events-none": !interactive })}
                  prefetch={links}
                >
                  <motion.g
                  key={component.identifier}
                  ...
                  onMouseEnter={() => interactive && setHoveredComponent(component.identifier)}
